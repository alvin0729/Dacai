...
#import "JczqGameInterfaceController.h"
...
@interface JczqGameInterfaceController ()
...
@implementation JczqGameInterfaceController
...
- (void)awakeWithContext:(id)context {
   ...
   //创建对象
    ...
    [self.wormhole listenForMessageWithIdentifier:@KWatchJCzqGameLivingArray listener:^(id messageObject) {
...
- (void)loadTableRows:(NSArray *)array{
    [self.interfaceTable setNumberOfRows:[array count] withRowType:@"defaultZqLiving"];
    [array enumerateObjectsUsingBlock:^(NSDictionary *rowData, NSUInteger idx, BOOL *stop) {
        ...
        [elementRow.leftLabel setText:rowData[@KWatchGameLivingHomeName]];
        [elementRow.rightLabel setText:rowData[@KWatchGameLivingAwayName]];
        
//            NSString *homeURL = rowData[@KWatchGameLiveHomeUrl];
//            NSString *awayURL = rowData[@KWatchGameLiveAwayUrl];
//
//            NSLog(@"home: %@", [[[homeURL pathComponents] lastObject] lowercaseString]);
//            NSLog(@"away: %@", [[[awayURL pathComponents] lastObject] lowercaseString]);
            ...
            if (homeImage) {
                ...
                [elementRow.leftImage setImage:homeImage];
            ...
            if (awayImage) {
                ...
                [elementRow.rightImage setImage:awayImage];
            } else {
...
//请求ios app响应
-(void)updateRequest{
    [WKInterfaceController openParentApplication:@{@"action": @KWatchJczqLiving} reply:^(NSDictionary *replyInfo, NSError *error) {
        if (error|| !replyInfo) {
            return ;
        }
    }];
...
- (void)table:(WKInterfaceTable *)table didSelectRowAtIndex:(NSInteger)rowIndex {
    ...
    [self pushControllerWithName:@"DPJczqGameInfoController" context:[NSNumber numberWithInteger:rowIndex]];
}
