//
//  DPBasketballCenterCommentController.m
//  Jackpot
//
...
@interface DPBasketballCenterCommentController () <UITableViewDelegate, UITableViewDataSource> {
@private
    NSMutableSet *_selectedSet;    //展开列表集合
    NSInteger _matchID;            //赛事id
    NSMutableArray *_dataArray;
    NSTimeInterval _lastTime;    //上次评论时间
}

@property (nonatomic, strong) DPBasketballCenterCommentViewModel *viewModel;
...
@implementation DPBasketballCenterCommentController
@dynamic viewModel;
...
- (instancetype)initWithMatchId:(NSInteger)matchId delegate:(id<DPDataCenterTableControllerDelegate>)delegate {
    if (self = [super initWithMatchId:matchId delegate:delegate]) {
        _title = @"评论";
        _matchID = matchId;
        _viewModel = [[DPBasketballCenterCommentViewModel alloc] initWithMatchId:matchId];
...
- (void)fetchFinished:(NSError *)error {
    ...
    if (self.isDownLoad) {
        ...
        [_dataArray addObjectsFromArray:self.viewModel.message.commentsArray];
        ...
        void (^block)(void) = ^() {
            ...
            [[AFHTTPSessionManager dp_sharedManager] POST:@"/datacenter/SubmitComment"
                ...
                success:^(NSURLSessionDataTask *task, id responseObject) {
                    DPLog(@"success ....");
                    @strongify(self);
                    PBMBasketballComment *comment = [PBMBasketballComment parseFromData:responseObject error:nil];
                    [_dataArray removeAllObjects];
                    [_dataArray addObjectsFromArray:comment.commentsArray];

                    [[DPToast makeText:@"评论成功"] show];
                    self.isDownLoad = NO;
                    self.viewModel.commendId = ((PBMComment *)[comment.commentsArray lastObject]).commentId;
                    [self addInfinitView];
                    [self.tableView reloadData];
