    ...
    switch (gameType) {
        ...
        case GameTypeZcNone: {
            ...
            for (int i = 0; i < 14; i++) {
                ...
                if (prettyStyle) {
                    label.backgroundColor = UIColorFromRGB(0x498643);
                    label.textColor = [UIColor dp_flatWhiteColor];
                    label.clipsToBounds = YES;
...
- (UILabel *)drawTimeLabel {
    if (_drawTimeLabel == nil) {
        ...
        _drawTimeLabel.font = [UIFont dp_systemFontOfSize:11];
    }
...
- (UIImageView *)arrowView {
    if (_arrowView == nil) {
        UIImage *downImage = [dp_CommonImage(@"black_arrow_down.png") dp_imageWithTintColor:[UIColor colorWithRed:0.71 green:0.67 blue:0.53 alpha:1]];
        UIImage *upImage = [dp_CommonImage(@"black_arrow_up.png") dp_imageWithTintColor:[UIColor colorWithRed:0.71 green:0.67 blue:0.53 alpha:1]];
...
- (void)buildLayout:(BOOL)exchangeTeam {
    ...
    if (exchangeTeam) {
        ...
        self.homeLabel.preferredMaxLayoutWidth = width + 5;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_delegateHas.titleAtIndex) {
        textLabel.text = [self.delegate view:self titleAtIndex:(int)indexPath.row];
    } else {
        textLabel.text = nil;
    }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (_delegateHas.didSelectedAtIndex) {
        [self.delegate view:self didSelectedAtIndex:(int)indexPath.row];
    }
}
...
#define DPResultInfoGirdViewWidth       (kScreenWidth - 20)
    ...
    if (self = [super initWithFrame:CGRectZero]) {
        ...
        for (int i = 0; i < row; i++) {
            for (int j = 0; j < column; j++) {
                 UILabel *label = [[UILabel alloc] init];
                label.backgroundColor = (i == 0 && gametype != GameTypeDlt) ? [UIColor colorWithRed:0.98 green:0.98 blue:0.95 alpha:1] : [UIColor dp_flatWhiteColor];
...
- (instancetype)initWithRow:(int)row column:(int)column {
    return [self initWithRow:row column:column gameType:GameTypeNone] ;
}
...
- (void)layoutSubviews {
    ...
    if (_gameType == GameTypeDlt) {
        if (_column == 1) {
            y = 0;
            width = DPResultInfoGirdViewWidth + 0.5 * (3 - 1);;
            itemWidth = floor(width / 3 * 0.6) + 0.5;

            for (int i = 0; i < _row; i++) {
                if (i != _row - 1) {
                    h = 2 * DPResultInfoGirdViewItemHeight - 0.5;
                } else
                    h = DPResultInfoGirdViewItemHeight;

                UILabel *label = _labels[i];
                label.frame = CGRectMake(0, y, itemWidth, h);

                y += h - 0.5;
            }
        } else {
            itemWidth = floor(width / _column);
            y = 0;
            h = DPResultInfoGirdViewItemHeight;
            ...
            for (int i = 0; i < _row; i++) {
                ...
                for (int j = 0; j < _column; j++) {
                    ...
                    CGFloat w = j == 0 ? ((width - itemWidth * (_column - 1)) - floor(width / 3 * 0.6)) : itemWidth;
...
- (CGSize)intrinsicContentSize {
    if (_gameType == GameTypeDlt) {
        if (_column == 1) {
            return CGSizeMake(floor(DPResultInfoGirdViewWidth/3*0.6), (_row*2-1) * DPResultInfoGirdViewItemHeight - 0.5 * (2*_row - 1));

        }
        
        return CGSizeMake((DPResultInfoGirdViewWidth - floor(DPResultInfoGirdViewWidth/3*0.6)), _row * DPResultInfoGirdViewItemHeight - 0.5 * (_row - 1));
...
- (void)setColors:(NSArray *)colors {
    for (int i =  _gameType == GameTypeDlt?0:1; i < _row; i++) {
        for (int j = 0; j < _column; j++) {
            UILabel *label = _labels[i * _column + j];
...
- (void)buildLayout:(void(^)(DPNumberResultInfoCell *cell))block {
    ...
    switch (self.gameType) {
        ...
        case GameTypeDlt: {
            ...
            DPResultInfoGirdView *detailTitleView = [[DPResultInfoGirdView alloc] initWithRow:1 column:3];
            DPResultInfoGirdView *MenuLeftView = [[DPResultInfoGirdView alloc] initWithRow:6 column:1 gameType:GameTypeDlt];
            DPResultInfoGirdView *MenuRightView = [[DPResultInfoGirdView alloc] initWithRow:11 column:3 gameType:GameTypeDlt];

            [self.contentView addSubview:titleView];
            [self.contentView addSubview:detailTitleView];
            [self.contentView addSubview:MenuLeftView];
            ...
            [detailTitleView mas_makeConstraints:^(MASConstraintMaker *make) {
                make.centerX.equalTo(self.contentView);
                make.top.equalTo(titleView.mas_bottom).offset(5);
            }];
            
            [MenuLeftView mas_makeConstraints:^(MASConstraintMaker *make) {
                make.left.equalTo(detailTitleView);
                make.top.equalTo(detailTitleView.mas_bottom).offset(-0.5) ;

            }];
 
            [MenuRightView mas_makeConstraints:^(MASConstraintMaker *make) {
                make.left.equalTo(MenuLeftView.mas_right);
                make.top.equalTo(detailTitleView.mas_bottom).offset(-0.5) ;
            }] ;
            
            
            [detailTitleView setTitle:@"奖项" forRow:0 column:0];
            [detailTitleView setTitle:@"中奖注数" forRow:0 column:1];
            [detailTitleView setTitle:@"每注金额（元）" forRow:0 column:2];
            ...
            for (int i=0; i<winTitles.count; i++) {
                [MenuLeftView setTitle:winTitles[i] forRow:i column:0];
            }
            
            
            NSArray *winDetails = @[@"基本", @"追加"];
            for (int i=0; i<11; i++) {
                [MenuRightView setTitle:winDetails[i%2] forRow:i column:0];
            }
            [MenuRightView setColors:@[[UIColor colorWithRed:0.58 green:0.56 blue:0.54 alpha:1], [UIColor colorWithRed:0.39 green:0.35 blue:0.31 alpha:1], [UIColor dp_flatRedColor]]];

            
            self.titleList = @[titleView];
            self.detailList = @[MenuRightView];
        }
            break;
        case GameTypeSd:
        case GameTypePs: {
    ...
    [self.titleList enumerateObjectsUsingBlock:^(DPResultInfoGirdView *obj, NSUInteger idx, BOOL *stop) {
        if (self.gameType == GameTypeZcNone) {
            ...
            [obj setTitle:@"本期销量(元)" forRow:0 column:0];
        }
        [obj setTitle:@"奖池(元)" forRow:0 column:1];
