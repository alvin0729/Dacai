...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        UIButton *subscribeBtn = [self dockItemWithTitle:@"订阅" tag:1 target:self action:@selector(dockItemClick:) seperatorLine:YES];
        UIButton *hotBtn = [self dockItemWithTitle:@"公告" tag:2 target:self action:@selector(dockItemClick:) seperatorLine:YES];
        UIButton *classfiyBtn = [self dockItemWithTitle:@"分类" tag:3 target:self action:@selector(dockItemClick:) seperatorLine:NO];
        ...
        _arrow = arrow;
        _dockItemArray = @[recommendBtn,subscribeBtn,hotBtn, classfiyBtn];
        
        UIView *superView = self;
        // 底部分割线
        bottomLine.backgroundColor = [UIColor dp_flatRedColor];
        ...
        [recommendBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(superView);
            make.right.equalTo(subscribeBtn.mas_left);
            make.top.equalTo(superView);
            make.bottom.equalTo(bottomLine.mas_top);
            make.width.equalTo(hotBtn);
            make.width.equalTo(subscribeBtn);
        ...
        [hotBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(superView);
            make.left.equalTo(subscribeBtn.mas_right);
            make.bottom.equalTo(bottomLine.mas_top);
...
{
    UIButton *btn = [UIButton dp_buttonWithTitle:title titleColor:[UIColor blackColor] backgroundColor:[UIColor clearColor] font:[UIFont systemFontOfSize:15]];
...
{
    [self selectedItemChangeTo:(int)sender.tag isDelegateSend:NO];
}
