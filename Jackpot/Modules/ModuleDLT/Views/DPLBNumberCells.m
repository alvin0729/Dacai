...
@implementation DPLBDigitalBallCell
...
//    CGFloat width=dp_DigitLotteryImage(@"ballNormal001_0.png").size.width;
    CGFloat spaceWidth=floor((kScreenWidth-32-7*ballHeight)/6) ;
    for (int i = 0; i < array.count; i++) {
        ...
        [button mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(contentView).offset(16+(ballHeight+spaceWidth)*(i%7));
            make.top.equalTo(self.titleLabel.mas_bottom).offset(10+ballHeight *(i/7));
...
- (UIButton *)createButton {
    ...
    if (self.ballsColor==1) {
        ...
        [button setBackgroundImage:dp_DigitLotteryImage(@"ballSelectedBlue001_14.png") forState:UIControlStateSelected];
    }
    [button setTitleColor:UIColorFromRGB(0x887768) forState:UIControlStateNormal];
...
- (UILabel *)titleLabel {
    if (_titleLabel == nil) {
        ...
        _titleLabel.font = [UIFont dp_systemFontOfSize:13.0];
    }
    return _titleLabel;
}
//改变top
...
{
    // 实现
    if ([dataModel isKindOfClass:[DPLBDigitalCellViewModel class]]) {
        DPLBDigitalCellViewModel *model = (DPLBDigitalCellViewModel *)dataModel;
        
        [self openOrCloseYilouZhi:model.isOpenSwitch];
        NSInteger count = model.btnSelectArray.count;
        for (int i = 0; i < count; i++) {
            UIButton *button=(UIButton *)[self.contentView viewWithTag:(self.lotteryType << 16) | i];
            button.selected = [model.btnSelectArray[i] intValue];
        }
        
        for (int i = 0; i < model.total; i++) {
            UILabel *label = (UILabel *)[self.contentView viewWithTag:(self.lotteryType << 10) | i];
            label.text = model.labelTextArray[i];
            label.textColor = (UIColor *)model.labelColorArray[i];
        }
    }
}
