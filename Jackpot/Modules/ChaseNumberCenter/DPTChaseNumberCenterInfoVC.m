...
#import "DPTChaseNumberCenterInfoVC.h"

#import "DPLTDltViewController.h"
#import "DPPayRedPacketVC.h"
...
@interface DPTChaseNumberCenterInfoVC () <UITableViewDelegate, UITableViewDataSource> {
@private
...
@property (nonatomic, strong) PBMChaseCenterInfo* dataBase;
@end

@implementation DPTChaseNumberCenterInfoVC
...
- (void)reqeustInfoForProejectDetail
{
    ...
    self.title = @"追号详情";
    //      self.title=[NSString stringWithFormat:@"方案%lld详情",self.projectId];
...
{
    if (self.dataBase.chaseInfoArray.count < 1) {
        return 1;
    }
    return 2;
}
...
- (void)tableView:(UITableView*)tableView didSelectRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    PBMChaseCenterInfo_ChaseCenterItem* item = [self.dataBase.chaseInfoArray objectAtIndex:indexPath.row];
    DPProjectDetailVC* controller = [[DPProjectDetailVC alloc] init];
...
- (NSMutableAttributedString*)dltChaseProjectDlt:(DPProjectDltBaseModel*)viewModel
{
    ...
    if ([viewModel isKindOfClass:[DPProjectDltDuplex class]]) {
    ...
    else if ([viewModel isKindOfClass:[DPProjectDltSingle class]]) {
    ...
    else if ([viewModel isKindOfClass:[DPProjectDltGallDrag class]]) {
    ...
    if (self.dataBase.payStatus == 1) {
        ...
        [[AFHTTPSessionManager dp_sharedManager] GET:@"/account/UsableRegBag"
            ...
            success:^(NSURLSessionDataTask* task, id responseObject) {
                ...
                DPPayRedPacketVC* vc = [[DPPayRedPacketVC alloc] init];
