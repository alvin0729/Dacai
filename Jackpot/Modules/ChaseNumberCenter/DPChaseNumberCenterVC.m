...
#import "DPChaseNumberCenterVC.h"
...
@interface DPChaseNumberCenterVC () <UITableViewDataSource, UITableViewDelegate>
//======================================view
/**
 *  contentView
 */
...
@implementation DPChaseNumberCenterVC
...
- (void)requestData
{
    ...
    [self.param setValue:[NSString stringWithFormat:@"%d", self.index] forKey:@"type"];
    ...
    [[AFHTTPSessionManager dp_sharedManager] GET:@"/account/FollowRecords"
        ...
        success:^(NSURLSessionDataTask* task, id responseObject) {
            ...
            if (self.refresh == topRefreshType) {
                ...
                [self.lotteryHistoryInfo.chaseCenterItemsArray addObjectsFromArray:chaseInfo.chaseCenterItemsArray];
            }

            switch (self.index) {
            case indexTypeChaseRuning:
                self.runArray = self.lotteryHistoryInfo.chaseCenterItemsArray;
                break;
            case indexTypeChaseUnPay:
                self.noPayArray = self.lotteryHistoryInfo.chaseCenterItemsArray;
                break;
            case indexTypeChaseFinished:
    ...
    if (!_contentView) {
        ...
        for (NSInteger i = 0; i < self.contentView.viewArray.count; i++) {
            ...
            _contentView.itemTappedBlock = ^(UIButton* btn) {
                ...
                self.index = (indexChaseType)[indexStr integerValue];
                switch (btn.tag) {
                case 100:
                    //                        self.type = @"1";
...
- (void)tableView:(UITableView*)tableView didSelectRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    PBMChaseCenterItem* item = [self.tableData objectAtIndex:indexPath.row];
    DPTChaseNumberCenterInfoVC* controller = [[DPTChaseNumberCenterInfoVC alloc] init];
