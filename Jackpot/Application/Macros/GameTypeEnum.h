...
typedef NS_ENUM(int, GameTypeId)
{
    GameTypeNone    = 0,
    ...
    GameTypeJcDg    = 1200, // 单关
    GameTypeJcDgAll = 1201, // 单关所有
...
inline static BOOL IsGameTypeJc(GameTypeId gameType) {
    return (gameType == GameTypeJcNone ||
            ...
            gameType == GameTypeJcHt ||gameType == GameTypeJcDgAll ||gameType== GameTypeJcDg||
...
//竞彩票样专用
inline static NSString *dp_GameTypeJCFullName(GameTypeId gameType) {
    switch (gameType) {
            // 数字彩
