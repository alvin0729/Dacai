...
@property (nonatomic, strong, readonly) UILabel  *attrLabel;
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [self.agreeButton mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerY.equalTo(self.attrLabel);
            make.right.equalTo(self.attrLabel.mas_left).offset(- 2);
...
- (void)setText:(NSString *)text {
    self.attrLabel.attributedText = nil;
...
- (void)pvt_onHandleTap {
    if (_target && _actionName) {
        ...
        if ([_target respondsToSelector:action]) {
            [_target performSelector:action];
        }
    }
}
...
- (UILabel *)attrLabel {
    if (_attrLabel == nil) {
        ...
        _attrLabel.userInteractionEnabled = YES;
    }
    return _attrLabel;
}
