...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    int prefix = GetPassModeNamePrefix(passModeTag), suffix = GetPassModeNameSuffix(passModeTag);
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.label.text = @"更多方式";
        self.label.textColor = [UIColor colorWithRed:0.55 green:0.49 blue:0.43 alpha:1];
        [self addSubview:self.label];
        
        self.button = [UIButton buttonWithType:UIButtonTypeCustom];
        [self.button setImage:dp_SportLotteryImage(@"transarrow_down.png") forState:UIControlStateNormal];
        [self.button setImage:dp_SportLotteryImage(@"transarrow_up.png") forState:UIControlStateSelected];
