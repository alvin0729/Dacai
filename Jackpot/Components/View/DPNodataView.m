...
-(void)setNoDataState:(DPNoDataState)noDataState{
    ...
    switch (noDataState) {
        ...
        case DPNoData:{
            ...
            }else if(self.gameType == GameTypeJcNone || self.gameType ==GameTypeZc14 ||self.gameType == GameTypeZc9 || self.gameType == GameTypeBdNone || self.gameType == GameTypeZcNone){
        ...
        case DPNoDataNoUpdateNormal:
        {
          self.noDataView.image =  dp_CommonImage(@"noDataFace.png") ;
             self.noDataView.attrString = kAttributStr(kNoData_) ;
        }
            break;

        case DPNodataDanGuan:{
            if (self.gameType == GameTypeLcNone) {
                self.noDataView.image =  dp_SportLotteryImage(@"no_basketBall.png") ;
            
            }else if(self.gameType == GameTypeJcNone ){
                self.noDataView.image = dp_SportLotteryImage(@"no_footBall.png") ;
            }
            self.noDataView.attrString = nil ;
        }break ;
