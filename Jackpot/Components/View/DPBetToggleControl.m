...
typedef NS_ENUM(NSInteger, DPBetToggleControlStyle) {
    DPBetToggleControlStyleHorizontal,//zuo
    DPBetToggleControlStyleVertical,
    DPBetToggleControlStyleHorizontal2,
    DPBetToggleControlStyleHorizontal3 ,
};
...
@property (nonatomic, strong, readonly) UILabel *titleLabel;//上或者左
@property (nonatomic, strong, readonly) UILabel *oddsLabel;//下或者右
@property (nonatomic, strong, readonly) UIImageView *backgroundView;

@property (nonatomic, assign) DPBetToggleControlStyle style;//展示类型
...
//                make.bottom.equalTo(self.titleLabel);
        ...
        case DPBetToggleControlStyleHorizontal3: {
            [self.titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
                make.centerY.equalTo(contentView);
...
+ (instancetype)horizontalControlForBasket{
    return [[DPBetToggleControl alloc] initWithStyle:DPBetToggleControlStyleHorizontal3];
 }
...
- (void)pvt_applyStyle {
    if (self.selected) {
        ...
        if (self.selectedColor) {
            self.oddsLabel.textColor=self.selectedColor;
