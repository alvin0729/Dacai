...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler position:(SVPullToRefreshPosition)position offsetY:(CGFloat)offsetY {
    
    if(!self.pullToRefreshView) {
        CGFloat yOrigin;
        switch (position) {
            case SVPullToRefreshPositionTop:
                yOrigin = -SVPullToRefreshViewHeight + offsetY;
                break;
            case SVPullToRefreshPositionBottom:
                yOrigin = self.contentSize.height + offsetY;
                break;
            default:
                return;
        }
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler position:(SVPullToRefreshPosition)position {
    [self addPullToRefreshWithActionHandler:actionHandler position:position offsetY:0];
}
    ...
    if(!showsPullToRefresh) {
        ...
        if (!self.pullToRefreshView.isObserving) {
            ...
            switch (self.pullToRefreshView.position) {
                case SVPullToRefreshPositionTop:
                    yOrigin = -SVPullToRefreshViewHeight + self.pullToRefreshView.offsetY;
                    break;
                case SVPullToRefreshPositionBottom:
                    yOrigin = self.contentSize.height + self.pullToRefreshView.offsetY;
                    break;
            }
...
//                                                      lineBreakMode:self.subtitleLabel.lineBreakMode];
        ...
        CGSize subtitleSize = [NSString dpsizeWithSting:self.subtitleLabel.text andFont:self.subtitleLabel.font andMaxSize:CGSizeMake(labelMaxWidth,self.subtitleLabel.font.lineHeight)];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if([keyPath isEqualToString:@"contentOffset"])
        ...
        switch (self.position) {
            case SVPullToRefreshPositionTop:
                yOrigin = -SVPullToRefreshViewHeight + self.offsetY;
                break;
            case SVPullToRefreshPositionBottom:
                yOrigin = MAX(self.scrollView.contentSize.height, self.scrollView.bounds.size.height) + self.offsetY;
                break;
        }
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != SVPullToRefreshStateLoading) {
        ...
        switch (self.position) {
            case SVPullToRefreshPositionTop:
                scrollOffsetThreshold = self.frame.origin.y - self.originalTopInset - self.offsetY;
                break;
            case SVPullToRefreshPositionBottom:
                scrollOffsetThreshold = MAX(self.scrollView.contentSize.height - self.scrollView.bounds.size.height, 0.0f) + self.bounds.size.height + self.originalBottomInset - self.offsetY;
                break;
        }
