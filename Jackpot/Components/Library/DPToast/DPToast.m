...
@interface DPToast () <KTMKeyboardObserver> {
@private
}
...
- (void)show {
    ...
    KTMKeyboardManager *manager = [KTMKeyboardManager defaultManager];
    if (manager.isKeyboardVisible) {
        [self setFrame:CGRectMake((kScreenWidth - self.dp_intrinsicWidth) / 2, kScreenHeight - self.dp_intrinsicHeight - kToastBottomMetric - CGRectGetHeight(manager.keyboardFrame), self.dp_intrinsicWidth, self.dp_intrinsicHeight)];
    } else {
...
- (void)keyboardFrameChanged:(KTMKeyboardTransition)transition {
    [UIView animateWithDuration:transition.animationDuration delay:0 options:transition.animationOptions animations:^{
        self.dp_y = CGRectGetMinY(transition.frameEnd)- self.dp_intrinsicHeight - kToastBottomMetric;
    } completion:nil];
}
