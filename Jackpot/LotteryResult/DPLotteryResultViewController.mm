...
@interface DPLotteryResultViewController () <UITableViewDelegate, UITableViewDataSource> {
@private
...
- (void)viewDidLoad {
    ...
    [self.tableView addPullToRefreshWithActionHandler:^{
        ...
        [strongSelf showHUD];
        [weakSelf dpn_rebindId:weakInstance->Net_RefreshHome() type:DRAW_REFRESH_HOME] ;
    }];
    [self.tableView.pullToRefreshView setDelay:0.15];
    
    [self showHUD];
    _cmdId = (int)[self dpn_rebindId:_resultInstance->Net_RefreshHome() type:DRAW_REFRESH_HOME];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    _resultInstance->GetHomeTarget((int)indexPath.row, gameType);
    _resultInstance->GetHomeTarget((int)indexPath.row, result, gameName, drawTime);
    _resultInstance->GetHomeTarget((int)indexPath.row, homeTeam, awayTeam, homeScore, awayScore, gameNameStr);
...
- (void)Notify:(int)cmdId result:(int)ret type:(int)cmdType {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (ret < 0) {
            [[DPToast makeText:[DPErrorParser CommonErrorMsg:ret]] show];
        }
