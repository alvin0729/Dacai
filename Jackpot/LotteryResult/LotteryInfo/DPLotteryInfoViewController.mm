...
//    }
    ...
    [recommendTableView addPullToRefreshWithActionHandler:^{
        [ weakSelf dpn_rebindId:weakInfoCenter -> Net_RefreshRecommend(true) type:Info_Recommend];
    } position:SVPullToRefreshPositionTop];
    
    [hotTableView addPullToRefreshWithActionHandler:^{
        [weakSelf dpn_rebindId:weakInfoCenter -> Net_RefreshAnnouncement(true) type:Info_Announcement];
    } position:SVPullToRefreshPositionTop];
    
    [recommendTableView addInfiniteScrollingWithActionHandler:^{
        [weakSelf dpn_rebindId:weakInfoCenter -> Net_RefreshRecommend() type:Info_Recommend];
    }];
    [hotTableView addInfiniteScrollingWithActionHandler:^{
       [weakSelf dpn_rebindId:weakInfoCenter -> Net_RefreshAnnouncement() type:Info_Announcement] ;
    }];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    if (tableView == _recommendTableView) {
        i = (int)_infoCenter -> GetRecommendCount();
    }else if (tableView == _hotTableView){
        i = (int)_infoCenter -> GetAnnouncementCount();
    }else{
         i = (int)_infoCenter -> GetLotteryCount();
    }
    return i;
}
...
- (DPLotteryInfoTableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == _recommendTableView) {
        _infoCenter -> GetRecommendTarget((int)indexPath.row, title, time, url, gameType);
    }else if (tableView == _hotTableView){
        _infoCenter -> GetAnnouncementTarget((int)indexPath.row, title, time, url, gameType);
    }
...
- (void)Notify:(int)cmdId result:(int)ret type:(int)cmdtype
{
    ...
    // 主线程刷新界面
        ...
        int rows = (int)[target numberOfRowsInSection:0];
...
- (void)refreshDataWithTargetTag:(int)tag {
    switch (tag) {
        case 0:
            [self dpn_rebindId:_infoCenter->Net_RefreshRecommend() type:Info_Recommend];
            break;
        case 1:
            [self dpn_rebindId:_infoCenter->Net_RefreshAnnouncement() type:Info_Announcement];
            break;
        case 2:
    ...
    if (_recTableNoDataView == nil) {
        ...
        [_recTableNoDataView setClickBlock:^(BOOL setOrUpDate){
            ...
            if (setOrUpDate) {
                ...
                [weakSelf dpn_rebindId:blockInfoInstance -> Net_RefreshRecommend() type:Info_Recommend];
               [weakSelf dpn_rebindId:blockInfoInstance -> Net_RefreshAnnouncement() type:Info_Announcement] ;
            }
