...
- (UMZoomScrollView *)zoomScrollViewWithPageIndex:(NSInteger)pageIndex imageLocation:(ImageLocation)imageLocation
{
    ...
    if (self.imageModels.count > 0) {
        ...
        if (self.imageModels.count > pageIndex) {
            ;
            UMComImageModel *imageModel = self.imageModels[pageIndex];
            [zoomView.imageView setImageURL:[NSURL URLWithString:imageModel.largeImageUrlString]];
            [zoomView.imageView setThumImageViewUrl:imageModel.smallImageUrlString];
        }
        return zoomView;
    }
    return nil;
