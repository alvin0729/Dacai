...
//    lpgr.delegate = self;
    ...
    [self.mTableView setSeparatorColor:[UIColor dp_flatRedColor]];
...
- (void)postFinishedWithError:(NSError *)error
{
    ...
    if ([[[UMFeedback sharedInstance] getUserInfo][@"is_failed"] boolValue]) {
        self.infoLabel.textColor = [UIColor dp_flatRedColor];
    } else {
...
- (UIView *)infoView {
    if (_infoView == nil) {
        ...
        if ([self.inputToolBar.contactInfo[@"is_failed"] boolValue]) {
            infoLabel.textColor = [UIColor dp_flatRedColor];
        } else {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (content.length > 0) {
        CGSize labelSize = [NSString dpsizeWithSting:content andFont:[UIFont systemFontOfSize:14.0f] andMaxWidth:self.mTableView.frame.size.width - 40];
